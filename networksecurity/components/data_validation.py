...
from networksecurity.constant.trainig_pipeline import SCHEMA_FILE_PATH
...
class DataValidation:
    ...
    def validate_no_of_columns(self,dataframe:pd.DataFrame) -> bool:
        try:
            ...
            logging.info(f"Number of columns reuired:{number_of_columns}")
    ...
    def detect_data_drift(self,base_df:pd.DataFrame,current_df:pd.DataFrame,threshod=0.05) -> bool:
        try:
            status=True
            report={}
            for column in base_df.columns:
                ...
                if threshod<=is_same_dist.pvalue:
                    is_found=False
    ...
    def initiat_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            if not status:
                error_message=f"Train dataframe does not all the columns. \n"
            
            status=self.validate_no_of_columns(dataframe=test_dataframe)
            if not status:
                error_message=f"Test dataframe does not all the columns. \n"
